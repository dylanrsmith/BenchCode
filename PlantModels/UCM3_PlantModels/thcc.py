...
class thcc_plant:
    ...
    def pos_to_time(self, pos):
        pos_time = round(pos*(60/50))  ## 60 is the seconds - time taken to complete 50mm distance
        return pos_time

    def pos_to_voltage(self, pos):
        volt = round(((3.32*pos)/169),2) ## 169 is the max potmeter value, 3.32 is max voltage 
        return volt
    
    def pos_to_pot_value(self, pos):
        pot_value = round(169-(81*(pos/50))) ## as the max voltage is equal to 0mm, the values are reversed and minused with 169.
    ...
    def calculate(self):
        if self._thcc.thcc_enable == 1:
            ...
            if self._thcc.thcc_breakaway_state == 1:
                if self._thcc.testing_active == 0:
                    if self._thcc.thcc_concave_inc == 1 and self._thcc.thcc_concave_dec == 0:
                    ...
                    elif self._thcc.thcc_concave_dec == 1 and self._thcc.thcc_concave_inc == 0:
            ...
            self._thcc.thcc_pot_volt = self.pos_to_voltage(self._thcc.thcc_pot)  ## convert pot meter to voltage
            self._io.data_to_board(591,self._thcc.thcc_pot)  ## Concave position sensor SPN
