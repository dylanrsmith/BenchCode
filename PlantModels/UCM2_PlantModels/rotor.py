...
class rotor_hydro:
    ...
    log_file_index = 0
    if log_enable == 1:
        log_file = open("/home/pi/Desktop/Bench_Code/PlantModels/UCM2_PlantModels/rotor_current_log.csv","a")
...
#             return 0
    ...
    def calculate_rotor(self):
        ...
        hydro_current_inc = self.getIncrCurrentFromvoltage(hydro_vlt_inc,1.2,1.2/0.736,RotorIncr)
        hydro_current_dec = self.getDecrCurrentFromvoltage(hydro_vlt_dec,1.2,1.0 / 1.2,RotorDecr)
        
        volumeRotationPump = self.getIncDisplacementVolume(hydro_current_inc)-self.getDecDisplacementVolume(hydro_current_dec)
...
#        print("\nHydro Motor Speed : ", self.HydroMotorSpeed)
        ...
        if (rtf_on or etr_on):
            ...
            self.RotorSpeed = self.RingTeeth / (self.SunTeeth + self.RingTeeth) * ringspeed * 67.0 / 59.0 + self.SunTeeth / (self.SunTeeth + self.RingTeeth) * self.HydroMotorSpeed            
        else:
