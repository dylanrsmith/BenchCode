...
#        self._io.data_to_board(263, int(0))
    ...
    def calculate_state(self):
        ...
        if self._gh.ghcv_enabled:                                 # To enable or disable the plant module from GUI section Plant Model UCM2
            if self._gh.testing_active == 0:
            
                if self._gh.iscoveropenactive == 1 and self._gh.iscoverclosedactive == 0: # only process data if state/edge is changed
                    # Covers Open Edge
                    if self._gh.g_opened_tmr_ms_u16 > 0:                               # Decrement cover open counter
                        self._gh.g_opened_tmr_ms_u16 -= 1
                    if self._gh.open_error != 1 and self._gh.g_opened_tmr_ms_u16 == 0: # if no open error set from GUI and open counter is 0
                        self._gh.g_closed_tmr_ms_u16 = self._gh.g_tmr_max_val_ms_u32   # Reset counters
                        ...
                        self._io.data_to_board(257, int(0))                            # Update status of open sol sensor
                        self._io.data_to_board(263, int(1))                            # Update status of closed sol sensor
                        
                if self._gh.iscoveropenactive == 0 and self._gh.iscoverclosedactive == 1: # only process data if state/edge is changed
                    # Covers Close Edge
                    if self._gh.g_closed_tmr_ms_u16 > 0:                                # Decrement cover close counter
                        self._gh.g_closed_tmr_ms_u16 -=1
                    if self._gh.close_error != 1 and self._gh.g_closed_tmr_ms_u16 == 0: # if no close error set from GUI and close counter is 0                    
                        self._gh.g_closed_tmr_ms_u16 = self._gh.g_tmr_max_val_ms_u32    # Reset counters
                        self._gh.g_opened_tmr_ms_u16 = self._gh.g_tmr_max_val_ms_u32
                        self._gh.cover_open_sensor   = 0                                # Update status of open sol for GUI
                        self._gh.cover_close_sensor  = 1                                # Update status of closed sol for GUI                           
                        self._io.data_to_board(257, int(1))                             # Update status of open sol sensor
                        self._io.data_to_board(263, int(0))                             # Update status of closed sol sensor         
