class hdhc_plant:
    ...
    def header_plates(self, hdr_width, feeder_angle_voltage, lateral_tilt_voltage):
        if (self.current_frd_angle_volt != feeder_angle_voltage) \
            ...
            self.lateral_tilt_angle = -((self.current_lateral_tilt_voltage - 2500.0) * 5.0 / (2500.0 - 1100.0) * (3.14) / 180.0)
            self.norm_halfheaderwidth = self.headerwidth_mm * self.Mm2PixelsScaling / 2
            self.norm_MaxExtension_mm = (self.current_MaxExtension_mm * self.Mm2PixelsScaling)
    ...
    def calculate(self):
        ...
        if self._hc.hdhc_lat_float_enabled == 1:
            ...
            self.hdhc_adc_to_cur_cw = self._hc.hdhc_lateral_float_cw * self.max_cur_cw / 1024 * 1000
            self.hdhc_adc_to_cur_ccw = self._hc.hdhc_lateral_float_ccw * self.max_cur_ccw / 1024 * 1000

            self.hdhc_deltaV = 0
            if self._hc.hdhc_lateral_float_cw > self.hdr_cw_valve_threshold_cur:
                self.hdhc_deltaV += (self.hdhc_adc_to_cur_cw - self.hdr_cw_valve_threshold_cur) * 0.3385 * 0.01  #The 0.01 multiplication is due to the plantmodel is running @10ms.
            if self._hc.hdhc_lateral_float_ccw > self.hdhr_hdr_ccw_threshold_cur:
                self.hdhc_deltaV -= (self.hdhc_adc_to_cur_cw - self.hdhr_hdr_ccw_threshold_cur) * 0.3385 * 0.01
        ...
        if self._hc.hdhc_lift_pressure_enabled == 1:
            if self._hc.testing_active == 0:
                ...
                elif self._hc.hdhc_feeder_angle_volt > 1900 and self._hc.hdhc_feeder_angle_volt < 4300:
        ...
        self._hc.hdhc_feeder_angle_volt_pot = self.volt_to_pot(self._hc.hdhc_feeder_angle_volt)
        self._hc.hdhc_lh_height_tilt_volt_pot = self.volt_to_pot(self._hc.hdhc_lh_height_tilt_volt)
        self._hc.hdhc_center_lh_height_volt_pot = self.volt_to_pot(self._hc.hdhc_center_lh_height_volt)
        self._hc.hdhc_center_rh_height_volt_pot = self.volt_to_pot(self._hc.hdhc_center_rh_height_volt)
        self._hc.hdhc_rh_height_tilt_volt_pot = self.volt_to_pot(self._hc.hdhc_rh_height_tilt_volt)
        self._hc.hdhc_lateral_position_volt_pot = self.volt_to_pot(self._hc.hdhc_lateral_position_volt)
        ####################################################################
