class Feeder_hydro:
    ...
    log_file_index = 0
    if log_enable == 1:
        log_file = open("/home/pi/Desktop/Bench_Code/PlantModels/UCM1_PlantModels/feeder.csv","a")
    ...
    def Get_Hydro_INCR_Cur(self, Hydro_INCR_Vlt , Hydro_INCR_Vlt_Offset,Hydro_INCR_Vlt_ScaleFactor,FeederIncr):
        if FeederIncr == 1:
    ...
    def Get_Hydro_DECR_Cur(self, Hydro_DECR_Vlt, Hydro_DECR_Vlt_Offset, Hydro_DECR_Vlt_ScaleFactor, FeederDecr):
        if FeederDecr == 1:
    ...
    def calculate_Feeder(self):
        ...
        self.Hydro_INCR_Cur = self.Get_Hydro_INCR_Cur(self.Hydro_INCR_Vlt, self.Hydro_INCR_Vlt_Offset, self.Hydro_INCR_Vlt_ScaleFactor,self.FeederIncr)
        self.Hydro_DECR_Cur = self.Get_Hydro_DECR_Cur(self.Hydro_DECR_Vlt, self.Hydro_DECR_Vlt_Offset, self.Hydro_DECR_Vlt_ScaleFactor,self.FeederDecr)

        self.VolumeRotationPump = self.Get_Hydro_INCR_Displacement(self.Hydro_INCR_Cur) - self.Get_Hydro_DECR_Displacement(self.Hydro_DECR_Cur)
        self.VolumeFloating = self.VolumeRotationPump * self._Feeder.current_spd  # 1000 #ERPM
        ...
        if self.State_Diagram_Value == 2:
            if (self.Prev_FeederIncr == 0 and self.FeederIncr == 1) or (self.Prev_FeederDecr == 0 and self.FeederDecr == 1):
        ...
        if self.State_Diagram_Value == 3:
            if self.Prev_FeederClutchOn == 1 and self.FeederClutchOn == 0 and self.AuxPTOValveOn == 1:
                self.State_Diagram_Value = 1
        ...
        if self.State_Diagram_Value == 1:   # No Feeder Engage Calculate Feeder RPM and Motor RPM using Feeder_Speed_Setpoint
            self.Feeder_RPM         = 0
            self.Feeder_Motor_RPM   = 0
            self._io.data_to_board(92, 200)
            self._io.data_to_board(94, 200)
            
            self.Cal_Feeder_Motor_RPM    = (((self.Feeder_Speed_Setpoint/self.Header_JackShaft_Gbx)*(self.Ring_Teeth+self.Sun_Teeth) - self.MAX_Engine_RPM * self.EngineCVTInputRatio * self.Ring_Teeth) / self.Sun_Teeth)

            self.Cal_Feeder_RPM = ((self.Cal_Feeder_Motor_RPM*self.Sun_Teeth+(self.Engine_RPM*self.EngineCVTInputRatio*self.Ring_Teeth)) / (self.Ring_Teeth + self.Sun_Teeth)) * (self.Header_JackShaft_Gbx)
        ...
        if self.State_Diagram_Value == 3:   # Feeder Engage and Write Cal_Feeder_Motor_RPM
            self.Feeder_RPM         = self.Cal_Feeder_RPM
